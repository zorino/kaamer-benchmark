...
align() {
    ...
    docker run --rm --network kaamer-bench -u 1000:1000 -v $(pwd):/data kaamer:0.6.0 wait-for-it.sh -t 600 ecod-db:8321 -- kaamer -mink 10 -minr 0.001 -search -h http://ecod-db:8321 -i sequences/h_name.fasta -t prot -o ./results/kaamer-0.6-res.h_name.k10-noalign.tsv -m 150000
    docker stop ecod-db
    docker rm ecod-db
...
bench_split() {
    ...
    docker network create kaamer-bench
    docker run --name ecod-db --network kaamer-bench -d -u 1000:1000 -v $(pwd):/data kaamer:0.6.0 kaamer-db -server -d sequences/h_name.fasta.kaamer-0.6
    docker run --rm --network kaamer-bench -u 1000:1000 -v $(pwd):/data kaamer:0.6.0 wait-for-it.sh -t 600 ecod-db:8321 -- bash -c "{ time kaamer -mink 10 -minr 0.001 -search -h http://ecod-db:8321 -i sequences/h_name.split-$split.fasta -t prot -o ./results/kaamer-0.6-res.h_name.k10-noalign.split-$split.tsv -m 150000; } 2> ./results/kaamer-0.6-res.h_name.k10-noalign.split-$split.time"
    docker stop ecod-db
    docker rm ecod-db
    docker network rm kaamer-bench
}
